...
interface OrderPanelProps {
  ...
  /** @deprecated Use marketState instead */
...
export function OrderPanel({
  ...
  const isPaused = marketState === "PAUSED";
  const isClosed = marketState === "CLOSED";
  const useP2P = isClosed;
  ...
  const { data: custodyData } = useReadContracts({
    contracts: custodyAddr && !useP2P && !isPaused ? [
      {
  ...
  const availableLiq = !useP2P && !isPaused && lpLiq !== undefined && reservedBal !== undefined
  ...
  const handleApproveAndOpen = async () => {
    if (!address || collateralNum <= 0 || isPaused) return;
      ...
      {isPaused && (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-2 px-4">
            ...
            <p className="text-sm font-pixel" style={{ color: "var(--arcade-orange)" }}>
              Data temporarily unavailable
            </p>
            <p className="text-xs font-body" style={{ color: "var(--dim-text)" }}>
              Trading is paused while waiting for price data to resume. This typically resolves within minutes.
            </p>
          </div>
        </div>
      )}

      {/* Normal order form — hidden when paused */}
      {!isPaused && (
        <>
          {/* Long/Short Toggle */}
